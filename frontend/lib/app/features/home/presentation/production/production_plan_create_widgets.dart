...
const String _aiDraftMissingFields =
    "Select estate, product, start date, and end date first.";
...
class _PlanAiSectionState extends ConsumerState<_PlanAiSection> {
  ...
  bool get _hasRequiredFields {
    // WHY: AI draft generation depends on estate/product/date context.
  ...
  Future<void> _generateDraft() async {
    ...
    if (!_hasRequiredFields) {
      ...
      AppDebug.log(
        _logTag,
        _aiDraftMissing,
        extra: {_extraErrorKey: _aiDraftMissingFields},
      );
    ...
    setState(() {
      // WHY: Disable the button while the request is in flight.
      _isGenerating = true;
    });

    // WHY: Log the generation tap for UX analytics.
    AppDebug.log(_logTag, _aiDraftAction);
    try {
      final payload = {
      ...
      if (mounted) {
        setState(() {
          // WHY: Re-enable the button after the request completes.
          _isGenerating = false;
        });
      }
    }
