/**
 * backend/scripts/preorder-confirm.test.js
 * ----------------------------------------
 * WHAT:
 * - Integration tests for POST /business/preorder/reservations/:id/confirm.
 *
 * WHY:
 * - Ensures successful payment confirmation moves holds to confirmed safely and idempotently.
 *
 * HOW:
 * - Uses real auth + role middleware with the business controller endpoint.
...
const TEST_DB_NAME_PATTERN =
  /^(preorder_confirm_test|pcf_[a-z0-9]+_[a-z0-9]+)$/;
const CONFIRM_ROUTE_TEMPLATE =
  "/business/preorder/reservations/:id/confirm";
...
async function postConfirm({
  ...
  return requestJson({
    routePath: `/business/preorder/reservations/${reservationId}/confirm`,
    token,
  });
}
...
async function createProduct({
  ...
  return Product.create({
    ...
    description: "confirm test product",
    price: 1000,
    stock: 0,
    isActive: false,
...
async function seedConfirmScenario({
...
test.before(async () => {
  if (!process.env.JWT_SECRET) {
    throw new Error(
      "JWT_SECRET is required for preorder confirm tests",
    );
  }
  testDbUri =
  ...
  debug(
    TEST_LOG_TAG,
    "Connecting confirm test database",
    {
      hasMongoUri: Boolean(
...
test("confirm succeeds and keeps reserved quantity unchanged", async () => {
  const scenario = await seedConfirmScenario(
    {
      reservationStatus: "reserved",
      reservedQuantity: 5,
      reservationQuantity: 3,
...
test("confirm is idempotent when reservation already confirmed", async () => {
  const scenario = await seedConfirmScenario(
    {
      reservationStatus: "reserved",
      reservedQuantity: 4,
  ...
  const second = await postConfirm({
    token: scenario.ownerToken,
    reservationId:
      scenario.reservationId,
  });
...
test("confirm rejects non-reserved statuses", async () => {
  const scenario = await seedConfirmScenario(
    {
...
test("customer cannot confirm another customer's reservation", async () => {
  const scenario = await seedConfirmScenario(
    {
      reservationStatus: "reserved",
      reservedQuantity: 5,
