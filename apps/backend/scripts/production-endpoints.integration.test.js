...
const TEST_LOG_TAG =
  "PRODUCTION_ENDPOINTS_STAGE0_TEST";
...
const FEATURE_FLAG_ENV_TO_CONFIG_KEY =
  Object.freeze({
    PRODUCTION_ENABLE_PLAN_UNITS:
      "enablePlanUnits",
    PRODUCTION_ENABLE_UNIT_ASSIGNMENTS:
      "enableUnitAssignments",
    PRODUCTION_ENABLE_PHASE_UNIT_COMPLETION:
...
function buildProgressiveFlagEnv({
  enabledEnvKeys = [],
}) {
  const enabledSet = new Set(
    enabledEnvKeys,
  );
  return FEATURE_FLAG_ENV_KEYS.reduce(
    (acc, envKey) => {
      acc[envKey] = enabledSet.has(envKey)
        ? "true"
        : "";
      return acc;
    },
...
function resolvePhaseGateState({
  ...
  const normalizedCompletedUnits = Math.max(
    0,
    Number(completedUnitCount || 0),
  );
  const remainingUnits = Math.max(
    0,
    normalizedRequiredUnits -
      normalizedCompletedUnits,
  );
  const isLocked =
    normalizedPhaseType === "finite" &&
    remainingUnits <= 0;
...
function hasIndexPrefix({
  schemaIndexes,
  prefixFields,
}) {
  return schemaIndexes.some(
    ([fields]) =>
...
function collectProductionRouteSignatures(
  router,
) {
  const stack = Array.isArray(
    router?.stack,
  ) ?
    router.stack
  : [];
...
function loadFeatureFlagsWithEnv({
  forcedEnv = {},
}) {
  const modulePath = require.resolve(
    "../config/production_feature_flags",
  );
  const previousValues = {};
  FEATURE_FLAG_ENV_KEYS.forEach(
    (key) => {
      previousValues[key] =
        process.env[key];
      // WHY: Force explicit env values so default behavior can be asserted deterministically.
      process.env[key] = Object.prototype.hasOwnProperty.call(
        forcedEnv,
        key,
      ) ?
        forcedEnv[key]
...
test(
  "stage0 feature flags default to false when env is unset",
  () => {
    debug(
      TEST_LOG_TAG,
      "Checking default-off feature flags",
    );
    const {
      PRODUCTION_FEATURE_FLAGS,
    } = loadFeatureFlagsWithEnv({
      forcedEnv: {},
    });
...
test(
  "business routes expose core production lifecycle endpoints",
  () => {
    debug(
      TEST_LOG_TAG,
      "Checking production endpoint scaffold",
    );
    const businessRoutes = require(
      "../routes/business.routes"
    );
    const routeSignatures =
      collectProductionRouteSignatures(
        businessRoutes,
      );
...
test(
  "PlanUnit schema enforces unique unit index within each plan",
  () => {
    debug(
      TEST_LOG_TAG,
      "Checking PlanUnit schema indexes",
    );
    const PlanUnit = require(
      "../models/PlanUnit"
    );
    const schemaIndexes =
      PlanUnit.schema.indexes();
    const hasUniquePlanUnitIndex =
      schemaIndexes.some(
        ([fields, options]) =>
          fields?.planId === 1 &&
          fields?.unitIndex === 1 &&
...
test(
  "ProductionTask schema supports indexed assigned unit ids",
  () => {
    debug(
      TEST_LOG_TAG,
      "Checking ProductionTask unit assignment schema indexes",
    );
    const ProductionTask = require(
      "../models/ProductionTask"
    );
    const assignedUnitPath =
      ProductionTask.schema.path(
        "assignedUnitIds",
      );
    assert.ok(
      assignedUnitPath,
      "ProductionTask.assignedUnitIds path is missing",
    );

    const schemaIndexes =
      ProductionTask.schema.indexes();
    const hasAssignmentIndex =
      schemaIndexes.some(
        ([fields]) =>
          fields?.planId === 1 &&
          fields?.phaseId === 1 &&
...
test(
  "ProductionPhaseUnitCompletion schema is idempotent per phase unit",
  () => {
    debug(
      TEST_LOG_TAG,
      "Checking ProductionPhaseUnitCompletion schema indexes",
    );
    const ProductionPhaseUnitCompletion = require(
      "../models/ProductionPhaseUnitCompletion"
    );
    const schemaIndexes =
      ProductionPhaseUnitCompletion.schema.indexes();
    const hasUniquePhaseUnitIndex =
      schemaIndexes.some(
        ([fields, options]) =>
          fields?.planId === 1 &&
          fields?.phaseId === 1 &&
...
test(
  "ProductionPhase schema supports explicit phase gate fields",
  () => {
    debug(
      TEST_LOG_TAG,
      "Checking ProductionPhase phase gate schema fields",
    );
    const ProductionPhase = require(
      "../models/ProductionPhase",
    );
    const phaseTypePath =
      ProductionPhase.schema.path(
        "phaseType",
      );
    const requiredUnitsPath =
      ProductionPhase.schema.path(
        "requiredUnits",
      );
    const minRatePerFarmerHourPath =
      ProductionPhase.schema.path(
        "minRatePerFarmerHour",
      );
    const targetRatePerFarmerHourPath =
      ProductionPhase.schema.path(
    ...
    assert.ok(
      phaseTypePath,
      "ProductionPhase.phaseType path is missing",
    );
    assert.deepEqual(
    ...
    assert.ok(
      requiredUnitsPath,
      "ProductionPhase.requiredUnits path is missing",
    );
    assert.equal(
...
test(
  "ProductionUnitTaskSchedule schema supports per-unit timing rows",
  () => {
    debug(
      TEST_LOG_TAG,
      "Checking ProductionUnitTaskSchedule schema indexes and enums",
    );
    const ProductionUnitTaskSchedule = require(
      "../models/ProductionUnitTaskSchedule",
    );
    const timingModePath =
      ProductionUnitTaskSchedule.schema.path(
        "timingMode",
      );
    ...
    const schemaIndexes =
      ProductionUnitTaskSchedule.schema.indexes();
    const hasUniqueTaskUnitIndex =
      schemaIndexes.some(
        ([fields, options]) =>
          fields?.planId === 1 &&
          fields?.taskId === 1 &&
...
test(
  "ProductionUnitScheduleWarning schema supports manager review warnings",
  () => {
    debug(
      TEST_LOG_TAG,
      "Checking ProductionUnitScheduleWarning schema indexes and enums",
    );
    const ProductionUnitScheduleWarning = require(
      "../models/ProductionUnitScheduleWarning",
    );
    const warningTypePath =
      ProductionUnitScheduleWarning.schema.path(
        "warningType",
      );
    const severityPath =
      ProductionUnitScheduleWarning.schema.path(
    ...
    const schemaIndexes =
      ProductionUnitScheduleWarning.schema.indexes();
    const hasPlanUnitCreatedAtIndex =
      schemaIndexes.some(
        ([fields]) =>
          fields?.planId === 1 &&
          fields?.unitId === 1 &&
...
test(
  "TaskProgress schema supports unit-scoped daily records",
  () => {
    debug(
      TEST_LOG_TAG,
      "Checking TaskProgress unit-scoped unique index",
    );
    const TaskProgress = require(
      "../models/TaskProgress",
    );
    const unitIdPath =
      TaskProgress.schema.path(
        "unitId",
      );
    const expectedPlotUnitsPath =
      TaskProgress.schema.path(
        "expectedPlotUnits",
      );
    const actualPlotUnitsPath =
      TaskProgress.schema.path(
        "actualPlotUnits",
      );
    assert.ok(
      unitIdPath,
      "TaskProgress.unitId path is missing",
    );
    assert.ok(
    ...
    const schemaIndexes =
      TaskProgress.schema.indexes();
    const hasTaskStaffWorkDateUnitIndex =
      schemaIndexes.some(
        ([fields, options]) =>
          fields?.taskId === 1 &&
          fields?.staffId === 1 &&
    ...
    assert.equal(
      ...
      true,
      "TaskProgress unique index (taskId, staffId, workDate, unitId) is missing",
    );
  },
);

test(
  "ProductionDeviationGovernanceConfig schema supports phase-order thresholds",
  () => {
    debug(
      TEST_LOG_TAG,
      "Checking ProductionDeviationGovernanceConfig schema fields and indexes",
    );
    const ProductionDeviationGovernanceConfig = require(
      "../models/ProductionDeviationGovernanceConfig",
    );
    const phaseThresholdByOrderPath =
      ProductionDeviationGovernanceConfig.schema.path(
    ...
    const schemaIndexes =
      ProductionDeviationGovernanceConfig.schema.indexes();
    const hasUniquePlanIdIndex =
      schemaIndexes.some(
        ([fields, options]) =>
          fields?.planId === 1 &&
          options?.unique === true,
      );
    const hasTemplateLookupIndex =
      schemaIndexes.some(
        ([fields, options]) =>
          fields?.businessId === 1 &&
...
test(
  "LifecycleDeviationAlert schema supports governance statuses and plan-unit status index",
  () => {
    debug(
      TEST_LOG_TAG,
      "Checking LifecycleDeviationAlert schema enums and indexes",
    );
    const LifecycleDeviationAlert = require(
      "../models/LifecycleDeviationAlert",
    );
    const statusPath =
      LifecycleDeviationAlert.schema.path(
        "status",
      );
    assert.ok(
      statusPath,
      "LifecycleDeviationAlert.status path is missing",
    );
    assert.deepEqual(
    ...
    const schemaIndexes =
      LifecycleDeviationAlert.schema.indexes();
    const hasPlanUnitStatusIndex =
      schemaIndexes.some(
        ([fields]) =>
          fields?.planId === 1 &&
          fields?.unitId === 1 &&
...
test(
  "ProductionPlan schema supports deterministic confidence fields",
  () => {
    debug(
      TEST_LOG_TAG,
      "Checking ProductionPlan confidence schema fields",
    );
    const ProductionPlan = require(
      "../models/ProductionPlan",
    );
    const baselineScorePath =
      ProductionPlan.schema.path(
        "baselineConfidenceScore",
      );
    const currentScorePath =
      ProductionPlan.schema.path(
        "currentConfidenceScore",
      );
    const baselineBreakdownPath =
      ProductionPlan.schema.path(
        "baselineConfidenceBreakdown",
      );
    const currentBreakdownPath =
      ProductionPlan.schema.path(
        "currentConfidenceBreakdown",
      );
    const lastTriggerPath =
      ProductionPlan.schema.path(
        "confidenceLastTrigger",
      );
    const recomputeCountPath =
      ProductionPlan.schema.path(
...
test(
  "production confidence service exposes deterministic trigger constants",
  () => {
    debug(
      TEST_LOG_TAG,
      "Checking production confidence service exports",
    );
    const confidenceService = require(
      "../services/production_confidence.service",
    );
...
test(
  "stage7 confidence visibility remains manager-only across confidence endpoints and plan payload shaping",
  () => {
    debug(
      TEST_LOG_TAG,
    ...
    assert.equal(
      /function\s+canViewConfidenceScores\s*\(/.test(
        controllerSource,
      ),
      true,
    ...
    assert.equal(
      /stripPlanConfidenceFields/.test(
        controllerSource,
      ),
      true,
...
test(
  "stage8 feature flags support progressive enablement without coupling",
  () => {
    debug(
      TEST_LOG_TAG,
      "Checking progressive feature-flag enablement",
    );
    ...
    FEATURE_FLAG_ENV_KEYS.forEach(
      (envKeyToEnable) => {
        ...
        Object.entries(
          FEATURE_FLAG_ENV_TO_CONFIG_KEY,
        ).forEach(
          ([envKey, flagKey]) => {
            assert.equal(
              PRODUCTION_FEATURE_FLAGS[
...
test(
  "stage8 phase gate contract locks finite phases only after required units complete",
  () => {
    debug(
      TEST_LOG_TAG,
      "Checking phase gate finite-lock contract",
    );

    const finiteLockedState =
      resolvePhaseGateState({
        phaseType: "finite",
        requiredUnits: 8,
...
test(
  "stage8 monitoring phases remain lifecycle-neutral even when completion counts are high",
  () => {
    debug(
      TEST_LOG_TAG,
      "Checking monitoring phase lifecycle-neutral contract",
    );
...
test(
  "stage8 phase gate snapshot maps persisted phases by planId+phaseOrder and trims locked draft tasks",
  () => {
    debug(
      TEST_LOG_TAG,
    ...
    assert.equal(
      /const\s+persistedByOrder\s*=\s*new\s+Map/.test(
        controllerSource,
      ),
      true,
    ...
    assert.equal(
      /persistedByOrder\.get\(\s*phaseOrder\s*\)/.test(
        controllerSource,
      ),
      true,
...
test(
  "stage8 completion count performance guard relies on planId+phaseId index prefix",
  () => {
    debug(
      TEST_LOG_TAG,
      "Checking completion count query index guard",
    );
    const ProductionPhaseUnitCompletion = require(
      "../models/ProductionPhaseUnitCompletion",
    );
...
test(
  "stage8 duplicate completion writes remain idempotent via unique phase-unit index",
  () => {
    debug(
      TEST_LOG_TAG,
      "Checking duplicate completion idempotency contract",
    );
    const ProductionPhaseUnitCompletion = require(
      "../models/ProductionPhaseUnitCompletion",
    );
...
test(
  "stage8 approval-time phase unit completion sync is wired and flag-gated",
  () => {
    debug(
      TEST_LOG_TAG,
      "Checking phase unit completion write-path wiring",
    );
    const controllerSource =
      fs.readFileSync(
        path.resolve(
    ...
    assert.equal(
      /enablePhaseUnitCompletion/.test(
        controllerSource,
      ),
      true,
    ...
    assert.equal(
      /ProductionPhaseUnitCompletion\.bulkWrite\s*\(/.test(
        controllerSource,
      ),
      true,
    ...
    assert.equal(
      /approved_progress_has_no_actual_work/.test(
        controllerSource,
      ),
      false,
    ...
    assert.equal(
      /progress_unit_context_missing/.test(
        controllerSource,
      ),
      false,
    ...
    assert.equal(
      /function\s+getCompletedUnitCount\s*\(/.test(
        controllerSource,
      ),
      true,
    ...
    assert.equal(
      /function\s+getCompletedUnits\s*\(/.test(
        controllerSource,
      ),
      true,
...
test(
  "stage8 deviation governance contract supports alert trigger status and unit freeze state",
  () => {
    debug(
      TEST_LOG_TAG,
    ...
    const deviationLockedPath =
      PlanUnit.schema.path(
        "deviationLocked",
      );
    assert.ok(
      deviationLockedPath,
      "PlanUnit.deviationLocked path is missing",
    );
    assert.equal(
...
test(
  "stage8 deviation governance runtime wiring seeds config, evaluates threshold breaches, and applies manual replan writes",
  () => {
    debug(
      TEST_LOG_TAG,
      "Checking deviation governance runtime wiring",
    );
    const controllerSource =
      fs.readFileSync(
        path.resolve(
    ...
    assert.equal(
      /DEVIATION_REPLAN_TASKS_INVALID/.test(
        controllerSource,
      ),
      true,
...
test(
  "stage8 confidence recompute remains trigger-driven through controller wrappers",
  () => {
    debug(
      TEST_LOG_TAG,
    ...
    const directPlanRecomputeCalls = [
      ...controllerSource.matchAll(
        /recomputePlanConfidenceSnapshot\s*\(/g,
      ),
    ].length;
    const directScopedRecomputeCalls = [
      ...controllerSource.matchAll(
        /recomputeConfidenceForActivePlans\s*\(/g,
      ),
    ].length;
    assert.equal(
    ...
    const usedTriggerNames = [
      ...controllerSource.matchAll(
        /CONFIDENCE_RECOMPUTE_TRIGGERS\.([A-Z_]+)/g,
      ),
    ].map((match) => match[1]);
    ...
    usedTriggerNames.forEach(
      (triggerName) => {
        assert.equal(
          allowedTriggerNames.has(
            triggerName,
          ),
          true,
...
test(
  "stage5 unit delay propagation is wired to approval and manager detail insights",
  () => {
    debug(
      TEST_LOG_TAG,
      "Checking stage5 per-unit delay propagation wiring",
    );
    const controllerSource =
      fs.readFileSync(
        path.resolve(
    ...
    assert.equal(
      /function\s+seedUnitTaskScheduleRows\s*\(/.test(
        controllerSource,
      ),
      true,
    ...
    assert.equal(
      /function\s+buildUnitScheduleInsightsForPlan\s*\(/.test(
        controllerSource,
      ),
      true,
...
test(
  "stage6 plan detail payload exposes governance rows for managers and redacts risk for non-managers",
  () => {
    debug(
      TEST_LOG_TAG,
      "Checking stage6 manager-only risk payload shaping",
    );
    const controllerSource =
      fs.readFileSync(
        path.resolve(
    ...
    assert.equal(
      /function\s+normalizeDeviationAlertForResponse\s*\(/.test(
        controllerSource,
      ),
      true,
    ...
    assert.equal(
      /deviationGovernanceSummary,\s*deviationAlerts/.test(
        controllerSource,
      ),
      true,
...
test(
  "stage5 task progress unit context is enforced for deterministic per-unit shifts",
  () => {
    debug(
      TEST_LOG_TAG,
      "Checking stage5 task-progress unit context enforcement",
    );
    const controllerSource =
      fs.readFileSync(
        path.resolve(
    ...
    assert.equal(
      /TASK_PROGRESS_UNIT_REQUIRED_FOR_MULTI_ASSIGN/.test(
        controllerSource,
      ),
      true,
    ...
    assert.equal(
      /TASK_PROGRESS_BATCH_ENTRY_CODE_UNIT_ID_REQUIRED/.test(
        controllerSource,
      ) &&
    ...
    assert.equal(
      /unitId:\s*effectiveUnitId\s*\|\|\s*null/.test(
        controllerSource,
      ),
      true,
    ...
    assert.equal(
      /buildBatchTaskProgressError\s*\(\s*\{[\s\S]*unitId/.test(
        controllerSource,
      ),
      true,
...
test(
  "assistant-turn degrades retryable provider failures into a safe 200 assistant turn",
  () => {
    debug(
      TEST_LOG_TAG,
    ...
    assert.equal(
      /shouldReturnAssistantRetryTurn/.test(
        controllerSource,
      ),
      true,
    ...
    assert.equal(
      /provider failure degraded to assistant retry turn/.test(
        controllerSource,
      ),
      true,
