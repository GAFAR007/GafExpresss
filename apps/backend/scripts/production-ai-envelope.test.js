 ...
 * ----------------------------------------------
 * WHAT:
 * - Contract tests for strict AI envelope parsing and loose schema recovery in production_plan_ai.service.
 *
 * HOW:
 ...
 * - Asserts success/partial outcomes for all envelope actions and strict schema recovery/failures.
 *
 * WHY:
 ...
 *   while still producing deterministic recoverable drafts for the UI.
 */
...
test(
  "plan_draft envelope with extra keys is recovered into ai_draft_success",
  async () => {
    debug(
      TEST_LOG_TAG,
...
test(
  "plan_draft envelope with incorrect days/weeks is recovered with computed range",
  async () => {
    debug(
      TEST_LOG_TAG,
    ...
    assert.equal(
      result.draft.summary.totalEstimatedDays,
      61,
    );
  },
);
