...
class _ProductDetailScreenState extends ConsumerState<ProductDetailScreen> {
  ...
  void _addToCart(Product product) {
    if (!product.isPurchasable) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text("Preview catalog items are not available for checkout"),
        ),
      );
      return;
    }
  ...
  Future<void> _startPaystackCheckout(Product product) async {
    if (!product.isPurchasable) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text("This preview item is not connected to live checkout"),
        ),
      );
      return;
    }
...
class _ProductDetailBody extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final canBuy = product.isPurchasable && product.stock > 0;
        ...
        children: [
          ...
          if (!product.isPurchasable) ...[
            ...
            Container(
              ...
              child: Text(
                "Catalog preview item. Checkout stays available for live inventory products only.",
          ...
          _InfoRow(
            label: "Catalog type",
            value: product.isPurchasable ? "Live product" : "Preview item",
          ),
          _InfoRow(
            label: "Production",
