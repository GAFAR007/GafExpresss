...
List<ProductSellingOption> _normalizeBusinessProductSellingOptions(
  Iterable<ProductSellingOption> values,
) {
  return normalizeSellingOptions(values);
}

bool _businessProductSellingOptionsEqual(
  List<ProductSellingOption> left,
  List<ProductSellingOption> right,
) {
  final normalizedLeft = _normalizeBusinessProductSellingOptions(left);
  final normalizedRight = _normalizeBusinessProductSellingOptions(right);
  if (normalizedLeft.length != normalizedRight.length) {
    return false;
  }

  for (var index = 0; index < normalizedLeft.length; index += 1) {
    final leftItem = normalizedLeft[index];
    final rightItem = normalizedRight[index];
    if (leftItem.signature != rightItem.signature ||
        leftItem.isDefault != rightItem.isDefault) {
      return false;
    }
  }
...
class _BusinessProductDetailScreenState
  ...
  void _applySellingOptions(List<ProductSellingOption> sellingOptions) {
    _sellingOptions = _normalizeBusinessProductSellingOptions(sellingOptions);
  }
  ...
  void _addSellingOption() {
    ...
    final nextOptions = _normalizeBusinessProductSellingOptions([
      ..._sellingOptions,
      pending,
    ]);
    ...
    if (nextOptions.length == previousCount) {
      _showSnack("That selling option already exists.");
    }
  }

  void _removeSellingOption(ProductSellingOption option) {
    setState(() {
      _sellingOptions = _normalizeBusinessProductSellingOptions(
        _sellingOptions.where((item) => item.signature != option.signature),
      );
    });
  }
  ...
  Future<void> _saveProduct(Product product) async {
    ...
    final subcategory = _subcategoryCtrl.text.trim();
    final brand = _brandCtrl.text.trim();
    final sellingOptions = _normalizeBusinessProductSellingOptions(
      _sellingOptions,
    );
    ...
    if (category.isEmpty || subcategory.isEmpty) {
      _showSnack("Category and subcategory are required.");
      return;
    }
    ...
    if (sellingOptions.isEmpty ||
        !sellingOptions.any((option) => option.isDefault)) {
      _showSnack("Add at least one selling option and set a default one.");
      return;
    }
    ...
    if (category.isNotEmpty && category != product.category) {
      payload["category"] = category;
    }
    if (subcategory.isNotEmpty && subcategory != product.subcategory) {
      payload["subcategory"] = subcategory;
    }
    if (brand != product.brand) {
      payload["brand"] = brand;
    }
    if (!_businessProductSellingOptionsEqual(
      sellingOptions,
      product.sellingOptions,
    )) {
      payload["sellingOptions"] = [
        for (final option in sellingOptions) option.toJson(),
      ];
    }
            ...
            return ListView(
              ...
              children: [
                ...
                BusinessProductSellingFields(
                  ...
                  suggestedPackageTypes: productPackageTypeSuggestions(
                    categoryLabel: _categoryCtrl.text,
                    subcategoryLabel: _subcategoryCtrl.text,
                    extraSuggestions: _sellingOptions.map(
                      (option) => option.packageType,
                    ),
                  ),
                  suggestedMeasurementUnits: productMeasurementUnitSuggestions(
                    categoryLabel: _categoryCtrl.text,
                    subcategoryLabel: _subcategoryCtrl.text,
                    packageType: _packageTypeCtrl.text,
                    extraSuggestions: _sellingOptions.map(
                      (option) => option.measurementUnit,
                    ),
                  ),
                  ...
                  onAddOption: _addSellingOption,
                  onSetDefault: _setDefaultSellingOption,
                  onRemoveOption: _removeSellingOption,
                ),
                const SizedBox(height: 12),
                _buildTextField(
                ...
                Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      _buildAuditRow(
                        "Brand",
                        product.brand.trim().isEmpty ? "—" : product.brand,
                      ),
                      _buildAuditRow(
                        "Selling options",
                        product.sellingOptions.isEmpty
                            ? "—"
                            : product.sellingOptions
