...
import 'package:frontend/app/features/home/presentation/tenant_rent_period_selector.dart';
...
class _TenantVerificationScreenState
  ...
  Future<void> _startTenantPayment({
    ...
    try {
      ...
      // WHY: Show coverage and autoReduced so user knows what they are paying for.
      final coverage = data["coverage"] as Map<String, dynamic>?;
      if (mounted && coverage != null) {
        final coversFrom = coverage["coversFrom"]?.toString();
        final coversTo = coverage["coversTo"]?.toString();
        final autoReduced = coverage["autoReduced"] == true;
        String message = "";
        if (coversFrom != null && coversTo != null) {
          final fromDate = DateTime.tryParse(coversFrom);
          final toDate = DateTime.tryParse(coversTo);
          message =
              "Covers ${formatDateLabel(fromDate, fallback: "—")} to ${formatDateLabel(toDate, fallback: "—")}.";
        }
        if (autoReduced) {
          message += message.isEmpty ? "" : " ";
          message +=
              "Your selection was reduced to stay within the maximum 36 months.";
        }
      ...
      // WHY: Do NOT refresh here. Money/coverage must only update after payment succeeds.
      // Refresh happens on PaymentSuccessScreen when user lands after Paystack.
    } catch (error) {
      final message = _extractErrorMessage(error);
    ...
    return Column(
      ...
      children: [
        ...
        if (_selectedRentPeriod != null &&
            getRentPeriodLimit(_selectedRentPeriod) != null)
          TenantRentPeriodSelector(
            rentPeriod: _selectedRentPeriod!,
            value: _selectedPeriodCount,
            onChanged: (v) {
              setState(() {
                _selectedPeriodCount = v;
              });
            },
          ),
