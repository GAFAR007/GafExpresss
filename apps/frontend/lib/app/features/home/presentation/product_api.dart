...
class ProductApi {
  static const int _publicReadMaxAttempts = 3;
  static const Duration _publicReadRetryDelay = Duration(milliseconds: 600);

  final Dio _dio;

  ProductApi({required Dio dio}) : _dio = dio;

  bool _shouldRetryPublicRead(DioException error) {
  ...
  Future<Response<dynamic>> _getPublicReadWithRetry(
    String path, {
    ...
    while (true) {
      try {
        ...
        final shouldRetry =
            attempt < _publicReadMaxAttempts && _shouldRetryPublicRead(error);
        ...
        await Future.delayed(_publicReadRetryDelay);
        attempt += 1;
      }
    }
  }
  ...
  Future<List<Product>> fetchProducts({
    ...
    final resp = await _getPublicReadWithRetry(
      "/products",
  ...
  Future<Product> fetchProductById(String id) async {
    ...
    final resp = await _getPublicReadWithRetry("/products/$id");
    final data = resp.data as Map<String, dynamic>;
  ...
  Future<PreorderAvailability> fetchPreorderAvailability(
    ...
    final resp = await _getPublicReadWithRetry(
      "/products/$productId/preorder-availability",
    );
    final data = resp.data as Map<String, dynamic>;
